...
func (q *Queries) ListSchoolsAll(ctx context.Context, arg ListSchoolsAllParams) ([]ListSchoolsAllRow, error) {
	rows, err := q.db.QueryContext(ctx, listSchoolsAll, arg.SortBy, arg.SortOrder)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListSchoolsAllRow{}
	for rows.Next() {
		var i ListSchoolsAllRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.City,
