...
FROM professors P
  JOIN faculties F ON P.faculty_id = F.id
  JOIN schools S ON P.school_id = S.id
WHERE LOWER(P.first_name) LIKE LOWER($1::text) OR LOWER(P.last_name) LIKE LOWER($1::text) OR LOWER(concat(P.first_name, ' ', P.last_name)) LIKE LOWER($1::text)
LIMIT 5
`
...
func (q *Queries) SearchProfessorsByName(ctx context.Context, name string) ([]SearchProfessorsByNameRow, error) {
	rows, err := q.db.QueryContext(ctx, searchProfessorsByName, name)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
